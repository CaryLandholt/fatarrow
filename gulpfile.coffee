...
useBackend  = env.backend? or isProd
...
windowsify  = (windowsCommand, nonWindowsCommand) -> if isWindows then windowsCommand else nonWindowsCommand
...
processCoffeeScript = ->
	deferred     = q.defer()
	sources      = ['**/*.coffee']
	sources      = sources.concat ['!**/*.spec.coffee', '!**/*.backend.coffee'] if useBackend
...
processJavaScript = ->
	deferred               = q.defer()
	sources                = SCRIPTS
	sources                = sources.concat ['!**/*.spec.js'] if isProd
	sources                = sources.concat ['!**/*.backend.js', '!**/angular-mocks.js'] if useBackend
...
processTypeScript = ->
	deferred     = q.defer()
	sources      = ['**/*.ts']
	sources      = sources.concat ['!**/*.spec.ts', '!**/*.backend.ts'] if useBackend
...
gulp.task 'test', ['build'], ->
	return true if isProd or useBackend
